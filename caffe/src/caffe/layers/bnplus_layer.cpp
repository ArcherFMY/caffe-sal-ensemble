...
namespace caffe {
  ...
  void BNplusLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  template <typename Dtype>
  void BNplusLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    ...
    if (this->blobs_.size() > 0) {
      ...
      shared_ptr<Filler<Dtype> > scale_filler(GetFiller<Dtype>(
          this->layer_param_.bnplus_param().scale_filler()));
      ...
      shared_ptr<Filler<Dtype> > shift_filler(GetFiller<Dtype>(
          this->layer_param_.bnplus_param().shift_filler()));
  ...
  template <typename Dtype>
  void BNplusLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    ...
    switch (this->layer_param_.bnplus_param().bnplus_mode()) {
    case BNplusParameter_BNplusMode_LEARN:
      ...
      caffe_add(buffer_blob_.count(), const_top_data,
          buffer_blob_.cpu_data(), top_data);
      break;
    case BNplusParameter_BNplusMode_INFERENCE:
      // scale
      caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, N_, C_, 1, Dtype(1),
          batch_sum_multiplier_.cpu_data(), scale_data, Dtype(0),
          spatial_variance_.mutable_cpu_data());
      ...
      break;
    default:
      LOG(FATAL) << "Unknown BNplus mode.";
    } 
  }

  template <typename Dtype>
  void BNplusLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(BNplusLayer);
#endif

  INSTANTIATE_CLASS(BNplusLayer);
  REGISTER_LAYER_CLASS(BNplus);
}  // namespace caffe
