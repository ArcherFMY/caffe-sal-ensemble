...
*/
template <typename Dtype>
class BNplusLayer : public Layer<Dtype> {
 public:
  explicit BNplusLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  ...
  virtual inline const char* type() const { return "BNplus"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 1; }
  // if the BNplusMode is "LEARN" mamximum 3 top blobs are available
  virtual inline int MaxTopBlobs() const {
    return (this->layer_param_.bnplus_param().bnplus_mode() ==
            BNplusParameter_BNplusMode_LEARN) ? 3 : 1;
  }
